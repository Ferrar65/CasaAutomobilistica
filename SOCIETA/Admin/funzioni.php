...
function stampaDati($dati)
{
    ...
    foreach ($dati as $chiave => $elemento) {
        ...
        echo "<td><button class=\"rounded\"  class=\"w3-btn w3-round-xlarge\" type=\"submit\" name=\"val\" value=" .
            $tot .
    ...
    foreach ($dati as $chiave => $elemento) {
        ...
        foreach ($elemento as $key => $value) {
            if (!isset($value)) {
                ...
                echo '<td class=\" text-success\"><img width="250" height="150" src="data:image/jpeg;base64,' .
                    base64_encode($dati[$i]['immagini']) .
                    '"/></td>';
            } else {
                echo "<td class=\" text-success\"> $value </td>";
            }
